...
use capi::common::*;
use transactions::bid_offer::BidOfferWrapper;

use error::{Error, ErrorKind};

ffi_fn! {
    fn dmbc_tx_bid_offer_create(
...
ffi_fn! {
    fn dmbc_tx_bid_offer_free(wrapper: *const BidOfferWrapper) {
        if !wrapper.is_null() {
            unsafe { Box::from_raw(wrapper as *mut BidOfferWrapper); }
        }
    }
}

ffi_fn! {
    fn dmbc_tx_bid_offer_into_bytes(
        wrapper: *mut BidOfferWrapper,
        length: *mut size_t,
        error: *mut Error 
    ) -> *const u8 {
        let wrapper = match BidOfferWrapper::from_ptr(wrapper) {
            Ok(wrapper) => wrapper,
            Err(err) => {
                unsafe {
                    if !error.is_null() {
