...
#[test]
fn trade_interemedary_assets_not_found() {
    let mut testkit = init_testkit();
    let api = testkit.api();
    let transaction_fee = 100;
    let units = 2;
...
#[test]
fn trade_interemedary_insufficient_assets() {
    let mut testkit = init_testkit();
    let api = testkit.api();
    let transaction_fee = 100;
    let units = 2;
    ...
    let meta_data = "asset";
    let (seller_public_key, seller_secret_key) = WalletMiner::new()
        .add_asset(meta_data, units - 1, get_trade_fee(trade_tax, trade_ratio))
        .mine(&mut testkit);
