...
use dmbc::currency::assets::{Fees, MetaAsset, AssetBundle, TradeAsset};
...
fn fees_for_transfer() {
    ...
    let (public_key, secret_key) = WalletMiner::new()
        .add_asset(meta_data, amount, asset_fee(tax))
        .mine(&mut testkit);
...
fn fees_for_trade() {
    ...
    let (seller_public_key, seller_secret_key) = WalletMiner::new()
        .add_asset(meta_data, units, asset_fee(tax))
        .mine(&mut testkit);
    ...
    let response = post_request(&api, &tx_trade);
    let mut expected = FeesTable::new();
