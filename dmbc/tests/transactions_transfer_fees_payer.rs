...
use dmbc::currency::transactions::components::{PM_TRANSFER_WITH_FEES_PAYER, PM_ALL_ALLOWED};
use dmbc::currency::wallet::Wallet;

#[test]
fn transfer_with_fees_payer() {
    let fixed = 10;
    ...
    println!("{}", serde_json::to_string(&tx_transfer).unwrap());
    ...
    assert_eq!(recipient_wallet.balance, coins);
    assert_eq!(recipient_assets, vec![asset]);

    let sender_wallet = api.get_wallet(&public_key);
    let expected_balance = balance + units * fixed - coins;
    ...
    let fees_payer_wallet = api.get_wallet(&fees_payer_pk);
    let expected_balance = balance - units * fixed - transaction_fee;
    let fees_payer_assets = api.get_wallet_assets(&fees_payer_pk).iter().map(|a| a.into()).collect::<Vec<AssetBundle>>();
    assert_eq!(fees_payer_wallet.balance, expected_balance);
    assert!(fees_payer_assets.is_empty());
}

#[test]
fn transfer_with_fees_payer_asset_not_found() {
    let fixed = 10;
    let transaction_fee = 1000;
    let config_fees = TransactionFees::with_default_key(0, 0, 0, 0, 0, transaction_fee);
    let permissions = TransactionPermissions::default();
...
#[test]
fn transfer_with_fees_payer_insufficient_funds() {
    let fixed = 10;
...
#[test]
fn transfer_with_fees_payer_insufficient_assets() {
    let fixed = 10;
    let transaction_fee = 1000;
    let config_fees = TransactionFees::with_default_key(0, 0, 0, 0, 0, transaction_fee);
    let permissions = TransactionPermissions::default();
